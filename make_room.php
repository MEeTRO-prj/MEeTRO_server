...
function createNewUser() {
  if (isset($_POST["OWNER_ID"]) && $_POST["OWNER_ID"] != "") {
   ...
   // mysql query
   $query = "INSERT INTO ROOM (ROOM_ID, OWNER_ID, RIDE_DATE, RIDE_TIME, TIME_TYPE, RAILWAY_ID, RIDE_ST, DEST_ST, CAR_NUM, UPDATED_DATE, USE_FLG) VALUES(NULL, '$owner_id', '$ride_date', '$ride_time', '$time_type', '$railway_id', '$ride_st', '$dest_st', 1, SYSDATE(), '1')";
   $result = mysql_query($query) or die(mysql_error());
   if ($result) {
    $response["error"] = false;
    $response["message"] = "Room created successfully!";
    $selected = mysql_query("SELECT ROOM_ID FROM ROOM WHERE OWNER_ID = '$owner_id' ORDER BY OWNER_ID DESC LIMIT 1");
